...
![Mytilus edulis, the common mussel. Figure courtesy of [Wikipedia](https://en.wikipedia.org/wiki/File:Miesmuscheln-2.jpg), [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/).](./images/01-regression/Miesmuscheln-2.jpg){width=25% fig-align="center"}

We have obtained a dataset of 17 mussels, where for each mussel we record the frequency of the Lap94 allele, and the distance to the Southport seabord. This dataset is shown in @fig-southport. 
...
@fig-southport-residuals (right) shows a residual plot based on this quadratic regression and indicates that the previously found pattern has mostly disappeared. Hence this model better describes the data. In subsequent sections we will investigate whether the model can be improved further.
:::
...
$$ {#eq-homosc}
The constant $\sigma$ is called the *residual standard deviation*. If we assume @eq-homosc, we are able to determine the conditional variance $\text{Var}(Y \mid X=1300)=\sigma^2$, because then it can be estimated based on the data for all lakes, as will be illustrated in the next paragraph.
...
summary(m_log)
```
The residual plots for this model are shown in @fig-log-cwd-residuals below.
...
Although the residuals follow relatively well the normal distribution, deviations from linearity and homoscedasticity emerge. The pattern in @fig-log-cwd-residuals seems to be parabolic and makes it necessary to include a second order term in the model, so that we obtain the following model:
...
Compared to @fig-log-cwd-residuals, the residual plots for this model show somewhat less of a pattern.
...
![Japanese larch, or *Larix kaempferi*. Image courtesy of [Wikipedia](https://en.wikipedia.org/wiki/Larix_kaempferi#/media/File:Larix-kaempferi.JPG), [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/)](./images/01-regression/Larix-kaempferi.jpeg){width="30%" fig-align="center"}
...
In particular, the output suggests that trees that differ 0.1\% in their nitrogen level, but have the same level of phosphorus, differ on average 0.12 m in length, regardless from the actual level of phosphorus. @fig-needles-no-interaction (left) illustrates indeed that the mean tree length increases at the same rate with an increase of the level of nitrogen, irrespective of the level of phosphorus in the needles. Similarly, @fig-needles-no-interaction (right) shows that the association between tree length and nitrogen is 123.8, no matter what the level of phosphorus is.
...
It is relatively laborious and costly to determine the proportion of body fat in a person. For that reason, a number of studies have been set up in the past in order to unravel the pattern between the true percentage of body fat and several, more easily measurable, surrogates. One of these studies measured for 20 healthy women between the ages of 25 and 34 years the proportion of body fat $Y$, the thickness of the triceps skin fold $X_1$, the thigh circumference $X_2$, and the midarm circumference $X_3$. Pairwise scatter plots for this dataset are shown in @fig-scattermatrix.

```{r}
#| label: fig-scattermatrix
#| fig-align: center
#| fig-cap: Scatterplot matrix for the bodyfat dataset.
...
@fig-outliers shows the possible influence of extreme observations, or outliers, on the regression line. Plot (a) shows a synthetic dataset consisting of 10 observations together with the regression line. The other plots show what happens to the regression line when one outlier, displayed in red, is added. In each case, we show the unmodified regression line (dashed line) and the regression line when the outlier is added (solid line).
...
#| label: fig-outliers
#| fig-align: center
#| fig-cap: "Influence of outliers on the regression line. Figure (a): original dataset and regression line. Figure (b): Outlier with high leverage but low influence on the regression line. Figure (c): Outlier with low leverage. Figure (d): Outlier with large leverage and large influence."
set.seed(12345)  
...
@fig-cooks-distance-adjusted confirms that there is now no longer a single observation that has an outsize influence on the results of the obtained model.
...
[^confounder]: In practice we seldom know for sure (unless in randomised studies) if a certain variable $C$ is the only confounder for the association between $X$ and $Y$; often there can be unmeasured confounders that are not included in the dataset and for which it thus is also impossible to correct.
