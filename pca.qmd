...
### The coefficients of the principal components

interpreting the coefficients of the PCs.

profile plot

```{r}
#| fig-cap: The profile plot for the bodyfat datasets shows that the first principal component consists of ...
library(tidyr)
library(tibble)
...
df <- as.data.frame(pc$rotation) %>%
  ...
  pivot_longer(c("PC1", "PC2", "PC3"), names_to = "Component", 
               values_to = "Loading")

df$Variable <- as.factor(df$Variable)
ggplot(df, aes(x = Variable, y = Loading, 
...
To see this idea in action, let's revisit the body fat dataset and perform a principal component analysis:
...
For datasets with many features, a _scree plot_ or _elbow plot_ can be helpful to identify high-variance principal components. In a scree plot, the amounts of variance are plotted in descending order, so that one can identify at a glance the amount of variability contributed by each principal component. Some scree plots also include the total amount of variability, $S_K$, as a function of $K$. 

Figure @fig-scree-bodyfat displays a scree plot for the body fat dataset. Note that there is no one-line command in R to make a scree plot, you'll have to make your own. This can be a bit fiddly...
...
n <- seq(1, length(var_explained))
df_var <- data.frame(n = n, v = var_explained, t = total_var)
ggplot(df_var) +
  geom_line(aes(x = n, y = v, color = "Per PC")) + 
  geom_point(aes(x = n, y = v, color = "Per PC")) + 
  geom_line(aes(x = n, y = t, color = "Cumulative")) +
  geom_point(aes(x = n, y = t, color = "Cumulative")) +
  ylim(c(0, 1)) +
  scale_color_manual(
    name = "Explained variance",
    breaks = c("Per PC", "Cumulative"),
    values = c("Per PC" = "cornflowerblue",
