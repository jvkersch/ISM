...
There are many techniques for classification, including logistic regression, K-nearest neighbors, linear discriminant analysis, support vector classification, and decision trees, each with their own advantages and disadvantages. In this chapter, we will focus on **logistic regression** and **K-nearest neighbors** (KNN). The main reference for this chapter is *An Introduction to Statistical Learning* (@jamesIntroductionStatisticalLearning2021, sections 4.1--4.3).
...
More precisely, KNN estimates the probability that $x$ belongs to class $j$ as
$$
  P(Y = j | X = x) = \frac{1}{K} \sum_{i = 1}^{K} I(y_i = j),
$$
where the sum is over the $K$ nearest data points $y_1, \ldots, y_K$ to $x$, and $I(y_i = j)$ equals 1 if $y_i = j$ and 0 otherwise. For example, in @fig-knearest, with $K = 5$ neighbors, the estimated probabilities are $P(Y = B \mid X = x) = 3/5 = 60\%$ and $P(Y = M \mid X = x) = 2/5 = 40\%$.

KNN has a number of attractive properties: it requires no explicit training phase, is robust to outliers, and can easily handle more than two classes. On the other hand, it is not very interpretable---it is hard to understand *why* a particular classification was made---and it can be memory-intensive, since all training data must be stored.

The plot below shows the decision boundary for a KNN classifier with $K = 5$ on the breast cancer dataset. The background color indicates the predicted class at each point, with the intensity reflecting the confidence of the prediction.
...
$$
which simplifies to $\hat{\pi} = \bar{Y}$. In other words, the maximum likelihood estimate for the probability of success is simply the proportion of successes in the data---a reassuring result.
...
The parameters $\alpha$ and $\beta$ are estimated using maximum likelihood, as described in @sec-mle. The likelihood function is the probability of observing the data given the parameters:
$$
...
The contour plot below shows the log-likelihood as a function of $\alpha$ and $\beta$, with the maximum likelihood estimate marked.
...
: Coefficients for the model with interaction term. {#tbl-interaction-coefficients}

The interaction between radius and texture is not significant.
...
$$
which follows a standard normal distribution under the null hypothesis $H_0: \beta = 0$. This test is reported directly in the R regression output (`summary`).
...
: Wald test results for the multiple logistic regression model. {#tbl-wald-test}
...
The deviance should be compared with the critical value $\chi^2_{2; 0.95} = 5.991$ to draw a conclusion.
...
plot_decision_boundary <- function(model, data, n = 100, n_classes = 2) {
  xgrid <- with(data, expand_grid(
    radius_mean = seq(0.95 * min(radius_mean), 1.05 * max(radius_mean), length.out = n),
    texture_mean = seq(0.95 * min(texture_mean), 1.05 * max(texture_mean), length.out = n)))
...
p1 <- plot_decision_boundary(m_multi, train)
p2 <- plot_decision_boundary(m_inter, train)
...
![The confusion matrix summarizes the performance of a classifier by counting true positives (TP), true negatives (TN), false positives (FP), and false negatives (FN).](./images/02-logistic-regression/confusion_matrix.png){#fig-confusion-matrix}
...
By varying $C$ from 0 to 1, sensitivity and specificity change continuously and trace out the **Receiver Operating Characteristic (ROC) curve**. The closer the ROC curve sticks to the upper left corner, the better the classifier performs. The ROC curve can also be used to compare different classifiers.
